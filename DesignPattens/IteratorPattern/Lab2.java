...
public class Lab2 extends javax.swing.JFrame
{
	...
	void JButtonAllSwimmers_actionPerformed(java.awt.event.ActionEvent event)
	{
		...
		Iterator itr = vectorlist.iterator();
		 while(itr.hasNext()) {
			 Swimmer swimmer = (Swimmer)itr.next();
	...
	{
		Vector vectorlist=slist.getVector();
		ReverseIterator itr= new ReverseIterator(vectorlist);
	...
	{
		Vector vectorlist = slist.getVector();
		SelectiveIterator sit= new SelectiveIterator(vectorlist);
		Predicate<Swimmer> i= (s)->s.getAge()>11;
		while(sit.hasNext()) {
			 Swimmer swimmer = (Swimmer)sit.next(i);
			 if(swimmer!=null)
